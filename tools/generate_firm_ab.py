#!/usr/bin/env python3
"""Generate firm A/B comparison artifacts (CSV + overlay figure).

This helper orchestrates the Python 2 simulation (via ``run_ab_demo``)
for ``run_id=0`` over 200 ticks, then emits:

* ``data/firm_ab_table.csv`` – raw metrics (inflation volatility, price dispersion)
  for baseline vs. LLM ON scenarios.
* ``figs/firm_ab_overlay.png`` – price dispersion overlay (OFF dashed, ON solid) with
  the final 50 ticks shaded.
...
RUN_ID = 0
NCYCLE = 200
METRICS = ("inflation_volatility", "price_dispersion")
...
def plot_overlay(results: dict, output_path: Path) -> None:
    ...
    off_series = baseline.get("metric_series", {}).get("price_dispersion", [])
    on_series = llm_on.get("metric_series", {}).get("price_dispersion", [])

    if not off_series or not on_series:
        raise RuntimeError("Missing price_dispersion series in simulation results")
    ...
    ax.set_title("Firm price dispersion – A/B overlay (run 0, 200 ticks)")
    ax.set_xlabel("Tick")
...
def main() -> None:
    root = repo_root()
    tmp_json = root / "artifacts" / "firm_ab_result.json"
    ...
    table_path = root / "data" / "firm" / "firm_ab_table.csv"
    figure_path = root / "figs" / "firm" / "firm_ab_overlay.png"

    write_table(results, table_path)
    plot_overlay(results, figure_path)

    tmp_json.unlink(missing_ok=True)
    print("Generated {} and {}".format(table_path.relative_to(root), figure_path.relative_to(root)))
