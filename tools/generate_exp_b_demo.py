#!/usr/bin/env python3
"""Generate experiment B (single-country wage moderation) demo artifacts.

This helper runs the Python 2 ``run_ab_demo`` with overrides that increase the
wage adjustment parameter ``upsilon`` for country 0 to 120% of its baseline
value starting at tick 50 (``run_id=0``, ``ncycle=250``). The OFF leg remains at
baseline settings, while the ON leg applies the policy switch at tick 50. As
with the Experiment A helper, you can either let this script launch the Decider
stub automatically or reuse a stub you already started in another terminal.

Outputs:

* ``data/expB_demo/expB_metrics.csv`` – summary metrics (wage dispersion,
  fill rate) for baseline vs. policy-on scenarios.
* ``data/expB_demo/expB_wage_dispersion_series.csv`` – long-form time series
  for wage dispersion, OFF vs. ON.
* ``data/expB_demo/expB_fill_rate_series.csv`` – long-form time series for
  vacancy fill rates.
* ``figs/expB_demo/expB_wage_dispersion_overlay.png`` – overlay plot with OFF
  dashed, ON solid, final 50 ticks shaded, and a marker at the policy switch
  (tick 50).

Run from the repository root::

    python3 tools/generate_exp_b_demo.py
...
SCENARIO_MAP = {"off": "baseline", "on": "policy_high"}
FINAL_SHADE = 50
DECIDER_URL = "http://127.0.0.1:8000/healthz"
SERIES_OUTPUTS = {
    "wage_dispersion": "expB_wage_dispersion_series.csv",
    "fill_rate": "expB_fill_rate_series.csv",
}
PLOT_SETTINGS = {
    "metric": "wage_dispersion",
    "ylabel": "Wage dispersion",
    "title": f"Experiment B – wage dispersion overlay (run {RUN_ID}, {NCYCLE} ticks)",
}
...
def main() -> None:
    root = repo_root()
    tmp_json = root / "artifacts" / "expB_result.json"
    ...
    data_dir = root / "data" / "expB_demo"
    figs_dir = root / "figs" / "expB_demo"

    write_summary_table(results, data_dir / "expB_metrics.csv")
    for metric, filename in SERIES_OUTPUTS.items():
        write_series(results, data_dir, metric, filename)
    plot_overlay(results, figs_dir / "expB_wage_dispersion_overlay.png")

    tmp_json.unlink(missing_ok=True)
    print("Generated experiment B artifacts under data/expB_demo and figs/expB_demo.")
