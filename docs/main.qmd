...
Figure `firm_ab.qmd#fig-firm-ab` collapses to a single trajectory: the deterministic stub rejects every firm pricing request, so inflation volatility and price dispersion are identical across OFF and ON (Table `firm_ab.qmd#tbl-firm-ab`). The accompanying rationale table `firm_ab.qmd#tbl-firm-why` also remains empty because no LLM calls succeed under the stub. This "stub parity" outcome is expected given equations 12–14 still rely on the baseline heuristic, and it confirms that the A/B harness keeps the legacy behaviour intact. We expect measurable divergence on RQ1—changes in inflation volatility or price dispersion, supported by non-trivial why-code shares—only after live, context-aware prompts replace the stub.

### Bank credit and spreads (RQ2)

Bank credit overlays behave the same way. Figure `bank_ab.qmd#fig-bank-ab` shows complete overlap between OFF and ON, while Table `bank_ab.qmd#tbl-bank-ab` reports matching average spreads and loan-to-output ratios. The rationale table `bank_ab.qmd#tbl-bank-why` likewise records zero live decisions, signalling that the stub continues to fall back to the legacy leverage heuristic. The bank stub returns deterministic fallbacks that mirror the baseline leverage rule (equations 26–27), so no qualitative "soft information" is present yet. This overlay therefore functions as a control; RQ2 will only be addressed once live prompts simulate narrative lending decisions.

### Wage bargaining (RQ3)

Wage bargaining is the only block that diverges under the stub. Figure `wage_ab.qmd#fig-wage-ab` shows the ON leg flattening relative to the adaptive baseline, and Table `wage_ab.qmd#tbl-wage-ab` records a collapse in wage dispersion (0.002 vs 0.144) alongside a higher vacancy fill rate (0.55 vs 0.35). The rationale table `wage_ab.qmd#tbl-wage-why` still reads as a stub note because the deterministic path never emits positive why-coded adjustments. This shift is therefore driven by guard clamping rather than learned heuristics: the stub emits a zero step and the guards enforce it, while the baseline continues the standard drift from equations 1 and 15. The result highlights how the guard stack alone can stabilise the ON path ahead of the live prompt work.
